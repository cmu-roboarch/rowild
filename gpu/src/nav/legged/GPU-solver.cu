...
GPULeggedRobotSolver::GPULeggedRobotSolver(LeggedRobot *leggedRobot)
    : p(leggedRobot)
{
    d = new DeviceData();
}

GPULeggedRobotSolver::~GPULeggedRobotSolver()
{
    // vector<node_t> nodes;
...
void GPULeggedRobotSolver::initialize(double heuristicWeight)
{
    ...
    initializeCUDAConstantMemory(
        ...
        (uint32_t)p->toID(p->ex(), p->ey()), heuristicWeight);
...
bool GPULeggedRobotSolver::solve()
{
...
void GPULeggedRobotSolver::getSolution(float *optimal, vector<int> *pathList)
{
...
bool GPULeggedRobotSolver::isPrime(uint32_t number)
{
...
vector<uint32_t> GPULeggedRobotSolver::genRandomPrime(uint32_t maximum, int count)
{

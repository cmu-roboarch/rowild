...
GPUDroneSolver::GPUDroneSolver(Drone *drone)
    : p(drone)
{
    d = new DeviceData();
}

GPUDroneSolver::~GPUDroneSolver()
{
    delete d;
...
void GPUDroneSolver::initialize(double heuristicWeight)
{
    ...
    initializeCUDAConstantMemory(
        p->mapX(), p->mapY(), p->mapZ(), p->ex(), p->ey(), p->ez(),
        (uint32_t)p->toID(p->ex(), p->ey(), p->ez()), heuristicWeight);
...
void GPUDroneSolver::getSolution(float *optimal, vector<int> *pathList)
{
...
bool GPUDroneSolver::isPrime(uint32_t number)
{
...
vector<uint32_t> GPUDroneSolver::genRandomPrime(uint32_t maximum, int count)
{

...
GPUCarRobotSolver::GPUCarRobotSolver(CarRobot *carRobot)
    : p(carRobot)
{
    d = new DeviceData();
}

GPUCarRobotSolver::~GPUCarRobotSolver()
{
    delete d;
...
void GPUCarRobotSolver::initialize(double heuristicWeight)
{
    ...
    initializeCUDAConstantMemory(
        p->mapX(), p->mapY(), p->ex(), p->ey(), p->ethetaIdx(),
        (uint32_t)p->toID(p->ex(), p->ey(), p->ethetaIdx()), heuristicWeight,
        p->getRobotLength(), p->getRobotWidth());
...
bool GPUCarRobotSolver::solve()
{
    ...
    for (int round = 0; ;++round) {
        ...
        if (optimalNodesSize) {
            dprintf("\t\tRound %d: Found one solution\n", round);
            ...
            if (flipFloat(pq.top().fValue) <= optimalDistance) {
                dprintf("\t\t\t Number of nodes expanded: %d\n", d->nodeSize->Value());
...
void GPUCarRobotSolver::getSolution(float *optimal, vector<int> *pathList)
{
...
bool GPUCarRobotSolver::isPrime(uint32_t number)
{
...
vector<uint32_t> GPUCarRobotSolver::genRandomPrime(uint32_t maximum, int count)
{
